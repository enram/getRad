...
test_that("multiple radars work", {
  ...
  expect_identical(lapply(pvl, \(x)x$radar), as.list(r))
})


test_that("multiple timestamps work", {
  skip_if_offline()
  expect_true(is.list(pvl <- get_pvol(c("fianj"), time = t <- (c(-300, -900) + as.POSIXct(Sys.Date())))))
  expect_true(all(unlist(lapply(pvl, bioRad::is.pvol))))
  expect_identical(lapply(pvl, \(x)x$datetime), as.list(t))
})
...
test_that("multiple timestamps and radars work", {
  ...
  expect_true(all(unlist(lapply(pvl, bioRad::is.pvol))))
  expect_identical(lapply(pvl, \(x)x$datetime), as.list(rep(t, each = 2)))
  expect_identical(lapply(pvl, \(x)x$radar), as.list(rep(r, 2)))
})
