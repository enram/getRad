...
get_pvol_cz <- function(radar, time, ...) {
  ...
  rlang::check_installed(c("lubridate", "tidyr", "xml2", "rhdf5"), "to read czech radar data")
  ...
  data.frame(base = urls) |>
    ...
    tidyr::unnest(file) |>
    dplyr::filter(file != "../") |>
    dplyr::mutate(time_chr = sub(".hdf", "", sub(".*_OKPR_", "", file))) |>
  ...
  if (!all(unlist(lapply(
    ...
    alert("Not all polar volumes have the same attributes")
  }
  pvol <- Reduce(
    function(x, y) {
      x$scans <- mapply(
